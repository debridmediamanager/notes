...
| `WrongTurn3.nzb.gz` | a 58 GB stored 7z in 54 volumes of 1 GiB (the last shorter) plus seven PAR2 files, a full UHD BluRay tree; gzipped because the NZB is 7 MB plain | the split-7z reader: the volumes must be joined and the header at the end of the last one read before anything lists, and that listing is what is timed (`sevenzip_list_s`, one per rep from a cleared layout); the 57.6 GB feature stream must then answer 16 MiB at 8 GiB with the recorded md5, which is a correctness check rather than a timing one. No throughput number — every volume of this release is missing about a fifth of its articles on this provider, so a timed read would measure hole discovery |
