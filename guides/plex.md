...
- Load/stuck detection: `CheckServerLoad` inspects `/activities`, tracks `library.update.section` progress per UUID (a changed progress *or* subtitle counts as progress), and `IsJobStuck` flags stale jobs by `librarySectionID` with a Title fallback. `GetServerActivitiesDetails` creates and retires the same trackers, because `CheckServerLoad` only runs while the worker has a job queued — a scan Plex started by itself used to get no tracker at all, so it could never be reported stuck and its elapsed time read as zero on every refresh. Both paths keep history for `library.update.section` only; tracking anything else here would flap against the worker's prune.
- **The activity subtitle is not the file a parked scan is blocked on.** Measured on the plex503 rig, 2026-09-15: the `library.update.section` subtitle is the scanner's enumeration pointer and runs ahead of the reads. With the mount refusing three files it named a fourth that had never been requested, and it named the blocked file for 290 ms out of a 56 s cycle. The dashboard shows it as "Plex reports" and does not link it to a release; what the scan is actually waiting on comes from zurg's own refusals (`internal/universal/refused_reads.go`), which is the only side that knows. The corpus is in `internal/handlers/dashboard/testdata/plex503-*`.
